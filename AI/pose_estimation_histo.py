...
#from color_histogram_pose import compute_region_histogram
...
class PoseEstimation:
    ...
    """  def detect_pose(self, frame):
        ...
        pose_annotated_frame, detected_parts, body_box = self.process_pose(frame)
        return pose_annotated_frame, detected_parts, body_box,(dimension_height, dimension_width)
    """
    ...
    def detect_pose(self, frame):
        ...
        for box in body_boxes:
            ...
            pts = box['points']
            xs, ys = zip(*pts)
            # 2) compute and clamp AABB to frame
            x0, x1 = max(0, min(xs)), min(frame_w - 1, max(xs))
            y0, y1 = max(0, min(ys)), min(frame_h - 1, max(ys))
            
           
            # 3) extract region & compute histogram
            region = frame[y0:y1, x0:x1]
            if region.size == 0:
                # empty region → no points
                box['hist_pts']   = np.zeros((0,3), dtype=np.float32)
                box['hist_sizes'] = np.zeros((0,),   dtype=np.float32)
            else:
                pts3d, sizes = compute_region_histogram(region)
                box['hist_pts']   = pts3d
                box['hist_sizes'] = sizes
            # 4) append bbox with histogram data
            bboxes.append({
                'segment': box['segment'],
                'bbox':    (x0, y0, x1, y1),
                'hist_pts': box['hist_pts'],
                'hist_sizes': box['hist_sizes']
            })
